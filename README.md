...
**As any user (we recommend creating a separate user called etherpad):**
  ...
  3. Change into the new directory containing the cloned source code: `cd
     etherpad-lite`
  4. Run `ep_etherpad-lite/bin/run.sh` and open http://127.0.0.1:9001 in your browser.

To update to the latest released version, execute `git pull origin`. The next
start with `ep_etherpad-lite/bin/run.sh` will update the dependencies.
...
git.
  ...
  2. With a "Run as administrator" command prompt execute
     `ep_etherpad-lite\bin\installOnWindows.bat`

Now, run `start.bat` and open http://localhost:9001 in your browser.

Update to the latest version with `git pull origin`, then run
`ep_etherpad-lite\bin\installOnWindows.bat`, again.

If cloning to a subdirectory within another project, you may need to do the
following:

  1. Start the server manually (e.g. `node ep_etherpad-lite/node/server.ts`)
...
settings files, you can pass the path to a settings file to `ep_etherpad-lite/bin/run.sh`
...
You can debug Etherpad using `ep_etherpad-lite/bin/debugRun.sh`.

You can run Etherpad quickly launching `ep_etherpad-lite/bin/fastRun.sh`. It's convenient for
developers and advanced users. Be aware that it will skip the dependencies
update, so remember to run `ep_etherpad-lite/bin/installDeps.sh` after installing a new
