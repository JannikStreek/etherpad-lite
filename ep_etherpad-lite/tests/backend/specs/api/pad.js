 ...
 * <basedir>/ep_etherpad-lite/tests/container/specs/api/pad.js
 *
  ...
  */
  ...
  describe('copyPadWithoutHistory', function () {
    ...
    it('source and destination attribute pools are independent', async function () {
      ...
      const newBodyep_etherpad-lite = '<strong>bold</strong>';
      const newBodyDst = '<em>italic</em>';
      await setBody(sourcePadId, newBodyep_etherpad-lite);
      await setBody(newPad, newBodyDst);
      await setBody(sourcePadId, `${newBodyep_etherpad-lite} foo`);

      let [ep_etherpad-liteHtml, dstHtml] = await Promise.all([getHtml(sourcePadId), getHtml(newPad)]);
      assert.match(ep_etherpad-liteHtml, new RegExp(newBodyep_etherpad-lite));
      ...
      [ep_etherpad-liteHtml, dstHtml] = await Promise.all([getHtml(sourcePadId), getHtml(newPad)]);
      assert.match(ep_etherpad-liteHtml, new RegExp(newBodyep_etherpad-lite));
      assert.match(dstHtml, new RegExp(newBodyDst));
    });
  });
