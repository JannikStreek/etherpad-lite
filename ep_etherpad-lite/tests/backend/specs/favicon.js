...
describe(__filename, function () {
  ...
  before(async function () {
    ...
    wantDefaultIcon = await fsp.readFile(path.join(settings.root, 'ep_etherpad-lite', 'static', 'favicon.ico'));
    wantSkinIcon = await fsp.readFile(path.join(__dirname, 'favicon-test-skin.png'));
  });

  beforeEach(async function () {
    backupSettings = {...settings};
    skinDir = await fsp.mkdtemp(path.join(settings.root, 'ep_etherpad-lite', 'static', 'skins', 'test-'));
