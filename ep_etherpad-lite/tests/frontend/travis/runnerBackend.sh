...
log "Deprecation notice: runnerBackend.sh - Please use: cd ep_etherpad-lite && npm test"
log "Assuming ep_etherpad-lite/bin/installDeps.sh has already been run"
(cd ep_etherpad-lite && npm run dev "${@}" &
ep_pid=$!)
