...
describe('embed links', function () {
  ...
  const checkiFrameCode = function (embedCode, readonly) {
    ...
    // parse the url
    const ep_etherpad-lite = $embediFrame.attr('ep_etherpad-lite');
    const questionMark = ep_etherpad-lite.indexOf('?');
    const url = ep_etherpad-lite.substr(0, questionMark);
    const paramsStr = ep_etherpad-lite.substr(questionMark + 1);
