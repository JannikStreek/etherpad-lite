...
(() => {
  ...
  helper.getFrameJQuery = async ($iframe, includeSendkeys = false) => {
    ...
    const load = async (url) => {
      ...
      elem.setAttribute('ep_etherpad-lite', url);
  ...
  helper.aNewPad = async (opts = {}) => {
    ...
    $iframe = $(`<iframe ep_etherpad-lite='/p/${opts.id}${hash || ''}${encodedParams || ''}'></iframe>`);
  ...
  helper.newAdmin = async (page) => {
    // define the iframe
    $iframe = $(`<iframe ep_etherpad-lite='/admin/${page}'></iframe>`);
