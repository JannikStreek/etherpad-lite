 ...
 * Usage -- see README.md
 *
 * Normal usage:                node ep_etherpad-lite/bin/plugins/checkPlugin.js ep_whatever
 * Auto fix the things it can:  node ep_etherpad-lite/bin/plugins/checkPlugin.js ep_whatever autofix
 * Auto fix and commit:         node ep_etherpad-lite/bin/plugins/checkPlugin.js ep_whatever autocommit
...
(async () => {
  ...
  const epep_etherpad-liteDir = await fsp.realpath(path.join(epRootDir, 'ep_etherpad-lite'));
  ...
  const checkFile = async (ep_etherpad-liteFn, dstFn, overwrite = true) => {
    const outFn = path.join(pluginPath, dstFn);
    const wantContents = await fsp.readFile(ep_etherpad-liteFn, {encoding: 'utf8'});
  ...
  await Promise.all(workflows.map(async (fn) => {
    await checkFile(`ep_etherpad-lite/bin/plugins/lib/${fn}`, `.github/workflows/${fn}`);
  }));
  await checkFile('ep_etherpad-lite/bin/plugins/lib/dependabot.yml', '.github/dependabot.yml');
  ...
  if (!files.includes('package.json')) {
    ...
    if (files.includes('.eslintrc.js')) {
      ...
      checkFile('ep_etherpad-lite/bin/plugins/lib/eslintrc.cjs', '.eslintrc.cjs', false);
    }
  ...
  if (!readMeFileName) {
    ...
    if (autoFix) {
      ...
      await fillTemplate('ep_etherpad-lite/bin/plugins/lib/README.md', `${pluginPath}/README.md`);
    }
  }
  ...
  if (!files.includes('CONTRIBUTING') && !files.includes('CONTRIBUTING.md')) {
    ...
    if (autoFix) {
      ...
      await fillTemplate('ep_etherpad-lite/bin/plugins/lib/CONTRIBUTING.md', `${pluginPath}/CONTRIBUTING.md`);
    }
  }
  ...
  if (!files.includes('LICENSE') && !files.includes('LICENSE.md')) {
    ...
    if (autoFix) {
      ...
      await fsp.copyFile('ep_etherpad-lite/bin/plugins/lib/LICENSE', `${pluginPath}/LICENSE`);
    }
  }
  ...
  if (!files.includes('.gitignore')) {
    ...
    if (autoFix) {
      ...
      const gitignore =
          await fsp.readFile('ep_etherpad-lite/bin/plugins/lib/gitignore', {encoding: 'utf8', flag: 'r'});
  ...
  try {
    ...
    assert.equal(d, epep_etherpad-liteDir);
  } catch (err) {
    execSync(`${npmInstall} --no-save ep_etherpad-lite@file:${epep_etherpad-liteDir}`, {stdio: 'inherit'});
  }
  // linting begins
  try {
    logger.info('Linting...');
