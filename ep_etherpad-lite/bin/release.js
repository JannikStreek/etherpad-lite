...
node ep_etherpad-lite/bin/release.js patch

*/
const usage =
    'node ep_etherpad-lite/bin/release.js [patch/minor/major] -- example: "node ep_etherpad-lite/bin/release.js patch"';
...
const pkg = readJson('./ep_etherpad-lite/package.json');
...
try {
  ...
  writeJson('./ep_etherpad-lite/package.json', pkg);

  // run npm version `release` where release is patch, minor or major
  run('npm install --package-lock-only', {cwd: 'ep_etherpad-lite/'});
  ...
  const pkglock = readJson('./ep_etherpad-lite/package-lock.json');
  pkglock.lockfileVersion = 1;
  writeJson('./ep_etherpad-lite/package-lock.json', pkglock);

  run('git add ep_etherpad-lite/package.json');
  run('git add ep_etherpad-lite/package-lock.json');
