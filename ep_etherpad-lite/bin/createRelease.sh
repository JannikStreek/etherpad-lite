...
# - Replace version of etherpad in ep_etherpad-lite/package.json
...
function modify_files {
  ...
  sed -i -r "s/(\"version\"[ ]*: \").*(\")/\1$VERSION\2/" ep_etherpad-lite/package.json
...
function create_release_branch {
  ...
  git add ep_etherpad-lite/package.json
...
function create_builds {
  ...
  cd etherpad-lite
  ep_etherpad-lite/bin/buildForWindows.sh
