...
try cd "${workdir}"
[ -f ep_etherpad-lite/package.json ] || fatal "failed to cd to etherpad root directory"
...
#try pnpm i "${TMP_FOLDER}"/ep_etherpad-lite/node_modules

try cd "${TMP_FOLDER}"
[ -f ep_etherpad-lite/package.json ] || fatal "failed to copy etherpad to temporary folder"
...
rm -rf ep_etherpad-lite/node_modules || true

#log "do a normal unix install first..."
#$(try cd ep_etherpad-lite && ./bin/installDeps.sh)
