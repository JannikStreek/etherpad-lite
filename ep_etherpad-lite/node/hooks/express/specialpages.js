...
exports.expressPreSession = async (hookName, {app}) => {
  ...
  app.get('/robots.txt', (req, res) => {
    let filePath =
        path.join(settings.root, 'ep_etherpad-lite', 'static', 'skins', settings.skinName, 'robots.txt');
    res.sendFile(filePath, (err) => {
      // there is no custom robots.txt, send the default robots.txt which dissallows all
      if (err) {
        filePath = path.join(settings.root, 'ep_etherpad-lite', 'static', 'robots.txt');
  ...
  app.get('/favicon.ico', (req, res, next) => {
    (async () => {
      ...
      const fns = [
        ...
        path.join(settings.root, 'ep_etherpad-lite', 'static', 'skins', settings.skinName, 'favicon.ico'),
        path.join(settings.root, 'ep_etherpad-lite', 'static', 'favicon.ico'),
      ];
      for (const fn of fns) {
        try {
