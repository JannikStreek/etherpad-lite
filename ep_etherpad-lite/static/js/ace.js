...
const Ace2Editor = function () {
  ...
  this.init = async function (containerId, initialCode) {
    ...
    // Some browsers do strange things unless the iframe has a ep_etherpad-lite or ep_etherpad-litedoc property:
    ...
    // ep_etherpad-litedoc is avoided because Firefox's Content Security Policy engine does not properly handle
    // 'self' with nested ep_etherpad-litedoc iframes: https://bugzilla.mozilla.org/show_bug.cgi?id=1721296
    outerFrame.ep_etherpad-lite = '../static/empty.html';
    ...
    // The iframe MUST have a ep_etherpad-lite or ep_etherpad-litedoc property to avoid browser quirks. See the comment above
    // outerFrame.ep_etherpad-litedoc.
    innerFrame.ep_etherpad-lite = 'empty.html';
    ...
    requireKernel.ep_etherpad-lite =
    ...
    for (const module of ['ace2_inner', 'ace2_common']) {
      ...
      script.ep_etherpad-lite = absUrl(`../javascripts/lib/ep_etherpad-lite/static/js/${module}.js` +
    ...
    require.setRootURI(absUrl('../javascripts/ep_etherpad-lite'));
