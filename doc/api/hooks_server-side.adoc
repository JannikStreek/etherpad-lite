...
Context properties:
  ...
  * `ep_etherpad_litePad`: The source Pad object.
...
Context properties:
  ...
  * `data`: Raw JSONable object from the `.etherpad` file. This data must not be
    modified.
  * `ep_etherpad_litePadId`: The pad ID used for the pad-specific information in `data`.
...
* `fileEnding`: The lower-cased filename extension from `ep_etherpad_liteFile` **with leading
  period** (examples: `'.docx'`, `'.html'`, `'.etherpad'`).
* `padId`: The identifier of the destination pad.
* `ep_etherpad_liteFile`: The document to convert.
