...
=== loadSettings
Called from: ep_etherpad-lite/node/server.ts
...
=== pluginUninstall
Called from: ep_etherpad-lite/static/js/pluginfw/installer.js
...
Called from: `ep_etherpad-lite/static/js/pluginfw/plugins.js`
...
Called from: `ep_etherpad-lite/node/hooks/express.js`
...
=== eejsBlock_`<name>`
Called from: ep_etherpad-lite/node/eejs/index.js
...
=== padInitToolbar
Called from: ep_etherpad-lite/node/hooks/express/specialpages.js
...
=== onAccessCheck
Called from: ep_etherpad-lite/node/db/SecurityManager.js
...
Called from `ep_etherpad-lite/node/db/AuthorManager.js`
...
Called from: `ep_etherpad-lite/node/db/Pad.js`
...
Called from: `ep_etherpad-lite/node/db/PadManager.js`
...
Context properties:
  ...
  * `ep_etherpad-litePad`: The source Pad object.
...
=== socketio
Called from: ep_etherpad-lite/node/hooks/express/socketio.js
...
Called from: `ep_etherpad-lite/node/hooks/express/webaccess.js`
...
Called from: `ep_etherpad-lite/node/handler/PadMessageHandler.js`
...
Called from: `ep_etherpad-lite/node/utils/ExportHtml.js`
...
=== exportFileName
Called from ep_etherpad-lite/node/handler/ExportHandler.js
...
Called from `ep_etherpad-lite/node/utils/ExportEtherpad.js` and
`ep_etherpad-lite/node/utils/ImportEtherpad.js`.
...
Called from `ep_etherpad-lite/node/utils/ExportEtherpad.js`.
...
Called from `ep_etherpad-lite/node/utils/ImportEtherpad.js`.
...
Context properties:
  ...
  * `data`: Raw JSONable object from the `.etherpad` file. This data must not be
    modified.
  * `ep_etherpad-litePadId`: The pad ID used for the pad-specific information in `data`.

=== import

Called from: `ep_etherpad-lite/node/handler/ImportHandler.js`
...
* `fileEnding`: The lower-cased filename extension from `ep_etherpad-liteFile` **with leading
  period** (examples: `'.docx'`, `'.html'`, `'.etherpad'`).
* `padId`: The identifier of the destination pad.
* `ep_etherpad-liteFile`: The document to convert.
* `ImportError`: Subclass of Error that can be thrown to provide a specific
  ...
  matching one of the https://github.com/ether/etherpad-lite/blob/1.9.6/ep_etherpad-lite/static/js/pad_impexp.js#L80-L86[known error identifiers].
